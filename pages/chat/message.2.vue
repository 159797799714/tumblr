							...
							<view class="my" v-if="row.msg.user_info.uid == my_data.id">
								...
								<view class="left"
									...
									<view v-if="row.msg.type == 3" class="bubble img">
										<image :src="staticPath + row.msg.content.full_movie" style="width: 320px;height: 240px">
										</image>
								...
								<view class="other" v-if="row.msg.user_info.uid != my_data.id">
									<!-- 左-头像 -->
									<view class="left">
										<image @longtap="addFriendChat(row)" @tap="goDetails(row.msg.user_info.uid, row)"
									...
									<view class="right"
										...
										<!-- 视频消息 -->
										<view v-if="row.msg.type == 3" class="bubble img">
											<image :src="staticPath + row.msg.content.full_movie" style="width: 320px;height: 240px"></image>
...
export default {
	...
	methods: {
		...
		msgDataHandle(data, type) {
			for (let i = 0, j = data.length, msg_img; i < j; i++) {
				if (data[i].type == 0 && data[i].msg.type == 2) {
					if (data[i].msg.content.full_url) {
						msg_img = this.staticPath + data[i].msg.content.full_url.replace('_thumb.', '.');
					} else {
						msg_img = this.staticPath + this.list_id + '/' + data[i].msg.content.url;
					}
					if (type) {
		...
		// 预览图片
		showPic1(msg) {
			console.log("msg预览2", msg)
			let url = ''
			if (msg.content.full_url) {
				url = this.staticPath + msg.content.full_url.replace('_thumb', '')
				uni.previewImage({
					indicator: "none",
					urls: [url]
				});
			} else {
				url = this.staticPath + this.list_id + '/' + msg.content.url.replace('_thumb', '');
				uni.previewImage({
					indicator: "none",
					urls: [url]
				});
