								...
								<view class="left"
									...
									<view v-if="row.msg.type==3" class="bubble img ">
										<image :src=" staticPath + row.msg.content.full_movie"
											style="max-height:unset!important;" mode="widthFix">
										</image>
							...
							<view class="other" v-if="row.msg.user_info.uid != my_data.id">
								<view class="select" v-if="redirectingMsg">
									<radio-group @change="(state) => redirectMessageCheck(state, row.msg)">
										<label for=""><radio :checked="redirectMsgs.indexOf(row.msg.id) !== -1"></radio>转发</label>	
									</radio-group>
								</view>
								<!-- 左-头像 -->
								<view class="left">
								...
								<view class="right"
									...
									<view v-if="row.msg.type==3" class="bubble img">
										<image :src="staticPath + row.msg.content.full_movie"
											style="max-height:unset!important;" mode="widthFix"></image>
...
<!-- 							<view class="list-box" @tap="videoCall(1)" v-if="msgList.type == 0">
		...
		<view v-if="showVedio" class="showVedio">
			...
			<video autoplay="true" 
				style="position: absolute;top:50%;left:50%;transform: translate(-50%,-50%);width: 100%;" id="myVideo" :src="videoPath" @error="videoErrorCallback">
