	...
	<view class="page" :style="{ overflow: 'hidden', '--custom-font-size': custom_font_size + 'px' }">
		...
		<view class="page-content">
			<view class="gong-gao-content" v-if="msgList.type == 1">
				<Marqueen v-if="false" :notice="[notice]" @click="goToNoticeDetail()"></Marqueen>
				...
				<view class="gong-gao-content-info" v-if="top_notice">
					<view class="iconfont-im icon-gonggao gong-gao-content-info-icon" style="color: #0000FF;"></view>
					<span style="font-size:12px;color: #0000FF;width: 18%;">群公告：</span>
					<text-scroll style="font-size:12px;color: #000" :text="top_notice" ref="scrollNotice"></text-scroll>
				</view>
				<!--<view class="member-image-logo">-->
			...
			<view class="content">
				...
				<scroll-view class="msg-list" :style="{ height: scrollHeight }" scroll-y="true"
					:scroll-with-animation="scrollAnimation" :scroll-into-view="scrollToView" @scrolltoupper="loadHistory"
					:upper-threshold="30" id="scrollview" :scroll-top="scrollTop" @scroll="scrollEven" @touchstart="hideDrawer(3)">
					<!--<view class="loading"-->
					...
					<view class="row" v-for="(row, index) in msgList.list" :key="index" :id="'msg' + row.msg.id">
						...
						<block v-if="row.type == 0">
							<!-- 自己发出的消息 -->
							<view class="my msg-item" v-if="row.msg.user_info.uid == my_data.id">
								<view class="select" v-if="redirectingMsg">
									<checkbox-group @change="(state) => redirectMessageCheck(state, row.msg)">
										<label for="">
											<checkbox></checkbox>
										</label>
									</checkbox-group>
								...
								<view class="left"
									...
									<image v-else src="/static/img/unread.png" mode="widthFix" style="width: 30upx;margin-right: 10upx;">
									</image>
									<!-- 文字消息 非图片-->
									...
									<view v-if="row.msg.type == 0 && isImage(row.msg.content.text)" class="bubble"
										...
										<image :src="row.msg.content.text" :style="{ 'width': '200px', 'height': '200px' }" mode="widthFix"
											:lazy-load="true" />
									</view>
									<!-- 语言消息 -->
									...
									<view v-if="row.msg.type == 2" class="bubble img" @tap="showPic1(row.msg)">
										<image :src="(staticPath + list_id + '/' + row.msg.content.url)"
											:style="{ 'width': row.msg.content.w + 'px', 'height': row.msg.content.h + 'px' }" mode="widthFix"
											:lazy-load="true" />
									</view>
									<!-- 视频消息 -->
									<view v-if="row.msg.type == 3" class="bubble img ">
										<image :src="staticPath + row.msg.content.full_movie" style="max-height:unset!important;"
											mode="widthFix">
										</image>
									...
									<view v-if="row.msg.type == 5" class="bubble red-envelope" @tap="openRedEnvelope(row.msg, index)">
										...
										<view class="tis">
											{{ (receive_list.indexOf(row.msg.content.rid)) < 0 ? '恭喜发财，大吉大利' : '已领取' }} </view>
												<view class="blessing">
													<view class="info">
										...
										<view v-if="row.msg.type == 10" class="bubble" @tap="openMap(row.msg.content)">
											<view class="address-flex">
												<view class="address-name">
													...
													<text class="text-gray">{{ row.msg.content && row.msg.content.address }}</text>
												</view>
												<image class="fun-icon" src="../../static/theme/default/position.png" mode="widthFix"></image>
											</view>
										</view>
										<!-- 文件 -->
										<view v-if="row.msg.type == 11" class="bubble"
											...
											<view class="address-flex">
												<view class="address-name">
													<text>{{ row.msg.content && row.msg.content.name }}</text>
													<text class="text-gray">{{ row.msg.content && row.msg.content.length }}kb</text>
												</view>
												<image class="fun-icon" src="../../static/theme/default/file.png" mode="widthFix"></image>
											</view>
										</view>

									</view>
									<!-- 右-头像 -->
									<view class="right right-avatar" @tap="goDetails(row.msg.user_info.uid, row)">
										...
										<view class='user_tips gl' v-if="row.msg.is_admin == 1">
											管理员
										</view>
										<view class='user_tips' v-else-if="row.msg.is_admin == 2">
											群主
										</view>
										<view class='user_tips' v-else></view>
									</view>
								...
								<view class="other" v-if="row.msg.user_info.uid != my_data.id">
									<view class="select" v-if="redirectingMsg">
										<radio-group @change="(state) => redirectMessageCheck(state, row.msg)">
											<label for="">
												<radio :checked="redirectMsgs.indexOf(row.msg.id) !== -1"></radio>转发
											</label>
										</radio-group>
									</view>
									<!-- 左-头像 -->
									...
									<view class="right"
										...
										<view v-if="row.msg.type == 0 && isImage(row.msg.content.text)" class="bubble"
											@tap="showPic2(row.msg.content.text)">
											<image :src="row.msg.content.text" :style="{ 'width': '200px', 'height': 'auto' }" mode="widthFix"
												:lazy-load="true" />
										</view>
										<!-- 语音消息 -->
										<view v-if="row.msg.type == 1" class="bubble voice" @tap="playVoice(row.msg)"
					...
					<swiper-item>
						<view class="list">
							...
							<view class="list-box" @tap="camera">
								<image class="fun-icon" src="../../static/theme/default/shooting_icon.png" mode="heightFix">
								</image>
								<view class="tool_text">拍摄</view>
							</view>
							<view class="list-box" @tap="chooseCard">
								<image class="fun-icon" src="../../static/theme/default/contact_icon.png" mode="heightFix">
								</image>
								<view class="tool_text2">名片</view>
							</view>
							<view class="list-box" @tap="sendPos">
								<image class="fun-icon" src="../../static/theme/default/location_icon.png" mode="heightFix">
								</image>
						...
						<view class="list" style="margin-top: 8upx;">
							...
							<view class="list-box" @tap="handRedEnvelopes" v-if="showRedEnvelop">
								<image class="fun-icon" src="../../static/theme/default/envelope_icon.png" mode="heightFix">
								</image>
							...
							<view class="list-box" @tap="showQiang()" v-if="msgList.type == 1 && vendor_status && is_main">
					...
					<swiper-item>
						<view class="list">
							...
							<view class="list-box" @tap="chooseLike">
								<image class="fun-icon" src="../../static/theme/default/collection_icon.png" mode="heightFix">
								</image>
			...
			<view class="input-box" :class="popupLayerClass" v-else>
				<view class="redirect-btn" style="width:100%;" @click="finishRedirect()" v-if="redirectingMsg">
					<button type="primary" plain="true">确认转发({{ redirectMsgs.length }})</button>
				</view>
				<!-- H5下不能录音，输入栏布局改动一下 -->
				<!-- #ifndef H5 -->
				<view class="voice">
				...
				<view class="textbox">
					...
					<view class="text-mode" :class="isVoice ? 'hidden' : ''">
						...
						<view class="box">
							<textarea auto-height="true" confirm-type="search" @confirm="sendText" @input="inputText" :value="textMsg"
		...
		<view v-if="showVedio" class="showVedio">
			...
			<video autoplay="true" style="position: absolute;top:50%;left:50%;transform: translate(-50%,-50%);width: 100%;"
				id="myVideo" :src="videoPath" @error="videoErrorCallback">
			...
			<view class="layer" @touchmove.stop.prevent="discard">
				<view class="open-redenvelope">
					...
					<view class="money" @tap="openRed(redenvelopeData)">
						...
						<image :class="luckbtn" src="/static/theme/default/my/red_2.png" style="width: 180upx; height: 180upx;">
						</image>
					</view>
...
import Marqueen from '@/components/marqueen/index.vue'
import {
	mapState,
	mapMutations,
} from 'vuex'
...
export default {
	...
	methods: {
		...
		redirectMessageCheck(state, msg) {
			let idx = this.findIndexForRedirectMsg(msg);
			if (idx !== -1) {
				this.redirectMsgs.splice(idx, 1);
			} else {
				this.redirectMsgs.push(msg);
			}
		},
		findIndexForRedirectMsg(msg) {
			for (let i = 0; i < this.redirectMsgs.length; i++) {
				if (this.redirectMsgs[i].id == msg.id) {
					return i;
				}
			}
			return -1;
		...
		finishRedirect() {
			...
			console.log('this:', this.msgList, this.list_id);
			let datas = {
				list_id: this.list_id,
				type: this.msgList.type,
				msg_id: this.redirectMsgs,
			};
			let firstMsg = this.redirectMsgs[0];
			uni.setStorageSync('data_msg', JSON.stringify(datas));
			this.redirectingMsg = false;
			this.redirectMsgs = [];
			uni.navigateTo({
				url: '../push/sendallsome?type=' + firstMsg.type + '&content=' + JSON
					.stringify(firstMsg.content),
			});
		},
		// 消息是否是图片
		isImage(con) {
		...
		goToNoticeDetail() {
			uni.navigateTo({
				url: '../set/message_more?list_id=' + this.list_id + "&type=notice",
			})
		},
		// 选择要禁言的人
		selectMember(row) {
		...
		initData() {
			let _this = this;
			_get.base(function () {
				_this.showRedEnvelop = _data.data('red_envelope');
				console.log('_this.showRedEnvelop', _this.showRedEnvelop);
			});
			...
			_get.getChatData({
				...
				is_action_data: 1,
				callback: function (chatData) {
					console.log('callback chat data:');
					_this.top_notice = chatData.top_notice;
				}
			});
			_hook.routeSonHook();
			// todo 选择@好友高度
			...
			_this.top_notice = chat_data.top_notice;
		...
		autoHeight(type, not_return) {
			...
			if (type == 'show') {
				if (this.Keyboard) {
					...
					this.scrollHeight = (_this.scrollInfo.totall_height - _this.Keyboard - _this.scrollInfo
						.input_box_height2) +
						'px';
				} else {
					this.scrollHeight = (_this.scrollInfo.totall_height - _this.scrollInfo.input_box_height1 - _this
						.scrollInfo
						.input_box_height2) + 'px';
				}
			} else {
				...
				uni.getSystemInfo({
					success: function (res) {
						...
						if (_this.Keyboard) {
							_this.scrollHeight = (res.windowHeight - _this.scrollInfo.Keyboard - _this
								.scrollInfo
								.input_box_height2) + 'px'
						} else {
							_this.scrollHeight = (res.windowHeight - _this.scrollInfo.input_box_height2) +
								'px'
						}
					}
				});
		...
		addMsgImgList(data) {
			...
			for (let i = 0, j = data.length, msg_img; i < j; i++) {
				if (data[i].type == 0 && data[i].msg.type == 2) {
					this.msgImgList.push(this.staticPath + this.list_id + '/' + data[i].msg.content.url.replace(
						'_thumb', ''));
				}
			}
		},
		...
		msgAction(id, user_id, content, type) {
			...
			//     break;
			// }
			let itemList = ['撤回', '收藏', '转发', '复制'];

			if (_this.msgList.is_action != 0 && _this.msgList.type == 1) {
				itemList.push('撤回用户所有发言');
			}
			...
			uni.showActionSheet({
				...
				success: function (res) {
					switch (res.tapIndex) {
						...
						case 2:
							console.log("转发");
							_this.redirectingMsg = true;
							console.log('转发:', content, type);
							// let datas = {
							// 	list_id: _this.list_id,
							// 	type: _this.msgList.type,
							// 	msg_id: id
							...
							// uni.setStorageSync('data_msg', JSON.stringify(datas));
							// uni.navigateTo({
							// 	url: '../push/sendallsome?type=' + type + '&content=' + JSON
							// 		.stringify(content),
							// });
							break;
						//收藏
						...
						case 4:
							...
							_get.cancelUserAllMessage(sendData, function (res) {
								uni.showToast({
									title: res.msg,
									icon: 'none',
									duration: 1500
		...
		handRedEnvelopes() {
			...
			if (this.vendor_status) url = 'message/hand_pin?list_id=' + this.list_id + '&msg_type=' + this.msgList
				.type;;
			uni.navigateTo({
				url: url
			});
				...
				for (let i = 0; i < this.emojiList.length; i++) {
					...
					for (let j = 0; j < row.length; j++) {
						...
						if (EM.alt == item) {
							...
							let imgstr = '<img style="width:22px;height:22px;" src="' + _data.data(
								'static_url') +
								'/static/emoji/' + EM.url + '">';
		...
		// 添加图片消息到列表
		addImgMsg(msg) {
			this.msgImgList.push(this.staticPath + this.list_id + '/' + msg.content.url.replace('_thumb', ''));
		...
		showPic(msg) {
			...
			let url = this.staticPath + this.list_id + '/' + msg.content.url.replace('_thumb', '');
			uni.previewImage({
				indicator: "none",
				urls: [url]
			});
		...
		toUserDetail(item) {
			...
			uni.navigateTo({
				url: ('../details/index?user_id=' + item.user_id + '&type=1&list_id=' + this.list_id +
					"&is_action=" +
					this.is_action),
			});
		},
		getServing() {
			//判断权限
			if (this.msgList.type == 1 && (this.msgList.is_action == 0) && this.group_state && this.group_state
				.can_shangmai == 0) {
				uni.showToast({
...
.other {
	.left {
		...
		.user_tips {
			...
			width: 100%;
			line-height: 1;
			background: linear-gradient(92deg, #F7CBFD 0%, #7758D1 100%);
			...
			&.gl {
				background: linear-gradient(92deg, #ffd0c4 0%, #ff634a 100%);
			}
